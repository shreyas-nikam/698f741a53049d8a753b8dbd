...
# Streamlit Application Specification: Credit Default Classification for Financial Professionals

## 1. Application Overview

The **Credit Default Classification** Streamlit application is designed for financial professionals, specifically credit risk analysts like Alex Chen at CapitalFlow Bank. It provides a hands-on workflow for building, comparing, and interpreting credit default prediction models, focusing on practical financial implications.

The application guides the user through the following high-level story flow:

1.  **Setting the Stage**: Introduction to the credit risk scenario and the importance of accurate Probability of Default (PD) models.
2.  **Data Preparation**: Loading and preprocessing consumer lending data, handling class imbalance, and splitting into training and testing sets. Discussion of data leakage.
3.  **Logistic Regression Model**: Training a traditional, interpretable Logistic Regression model (credit scorecard baseline) and extracting odds ratios.
4.  **Random Forest Model**: Training a more flexible Random Forest classifier, incorporating SMOTE for imbalance handling and hyperparameter tuning.
5.  **Model Comparison & Evaluation**: Benchmarking models using credit-appropriate metrics like ROC-AUC, Precision-Recall curves, and KS statistic.
6.  **Cost-Sensitive Analysis**: Optimizing classification thresholds based on asymmetric misclassification costs (False Negatives vs. False Positives) to minimize financial losses.
7.  **Feature Importance**: Comparing feature importance from both models to understand key default drivers.
8.  **PD Calibration**: Assessing and, if necessary, calibrating model-predicted probabilities to ensure they reflect true observed default rates for accurate pricing and provisioning.

This application simulates a real-world workflow, emphasizing decision-making with financial context over mere algorithmic accuracy.
...
from source import (
    ...
    plot_roc_pr_ks,
    perform_cost_sensitive_analysis,
    plot_cost_curve,
    plot_financial_confusion_matrix,
    analyze_feature_importance,
    plot_feature_importance_bars,
    plot_predicted_pd_distributions,
    perform_pd_calibration,
    plot_calibration_curves
)
import pandas as pd # Explicitly include for st.dataframe if needed, or assume source handles returns as pandas objects.
import numpy as np # Explicitly include for general array ops if not fully encapsulated in source
import matplotlib.pyplot as plt # For st.pyplot, assumed plotting functions return figures
```

**Note on `source.py` functions**: It is assumed that the `source.py` file has been refactored to encapsulate the notebook's logic into the named functions listed above, with each function returning the necessary data or `matplotlib.figure.Figure` objects as described in the specification below.
...
if st.session_state.page == "Home":
    # Home Page Content
    # ... (see markdown and interaction details below)
    ...
    # Logistic Regression Page Content
    # ...
elif st.session_state.page == "3. Random Forest Model":
    # Random Forest Page Content
    # ...
elif st.session_state.page == "4. Model Comparison & Evaluation":
    # Model Comparison Page Content
    # ...
elif st.session_state.page == "5. Cost-Sensitive Analysis & Optimization":
    # Cost-Sensitive Analysis Page Content
    # ...
elif st.session_state.page == "6. Feature Importance":
    # Feature Importance Page Content
    # ...
elif st.session_state.page == "7. PD Calibration":
    # PD Calibration Page Content
    # ...
```

### `st.session_state` Design

`st.session_state` will be used to store data, models, and results generated across different steps of the workflow. This ensures that calculations performed on one "page" are available for subsequent steps without re-running expensive computations unnecessarily.
...
if 'metrics_calculated_default_th' not in st.session_state:
    ...
    st.session_state.metrics_df_default_th = None # Metrics at 0.5 threshold
...
if 'cost_analysis_performed' not in st.session_state:
    ...
    st.session_state.metrics_df_optimal_th = None # Metrics at optimal threshold
    st.session_state.thresholds_costs_data = None # For cost curve plot
...
if 'feature_importance_analyzed' not in st.session_state:
    ...
    st.session_state.log_importance_df = None # For bar chart
    st.session_state.rf_importance_df = None   # For bar chart
...
**Reading and Updating `st.session_state`:**

*   **Read**: On entering any page, relevant data and models will be read from `st.session_state` to display results or enable subsequent calculations.
*   **Update**: When a user triggers a computation (e.g., via a button click), the results of the `source.py` function call will be stored back into the appropriate `st.session_state` keys.

### Markdown, Widgets, and Function Invocation Points
...
*   No direct function invocations on this page. The user navigates to the next step.
...
if st.button("Load and Preprocess Data"):
    with st.spinner("Loading and preprocessing data..."):
        ...
        # Display initial stats from source.py output
...
if not st.session_state.data_loaded:
    st.warning("Please load and preprocess data first on the 'Data Preparation' page.")
elif not st.session_state.log_model_trained: # Assume LR is a prerequisite
    ...
    if st.button("Train Random Forest Model (with SMOTE & GridSearchCV)"):
        with st.spinner("Training Random Forest model with SMOTE and GridSearchCV (this may take a while)..."):
            # Note: X_train_sc and y_train are needed for SMOTE, X_test_sc for prediction
            ...
            # Display best parameters from source.py's print statements
            st.markdown(f"Best Random Forest Parameters: `{rf_grid_search.best_params_}`") # Assuming rf_grid_search is global in source
            st.markdown(f"Best Random Forest CV AUC: `{rf_grid_search.best_score_:.4f}`") # Assuming rf_grid_search is global in source
...
# Practitioner Warning Box for Accuracy
...
if not st.session_state.rf_model_trained:
    ...
    if st.session_state.metrics_calculated_default_th:
        ...
        # Plotting assumes source.py functions return matplotlib figures
        with st.spinner("Generating plots..."):
            fig_curves = plot_roc_pr_ks(st.session_state.y_test, st.session_state.y_prob_log, st.session_state.y_prob_rf)
            st.pyplot(fig_curves)
            plt.close(fig_curves) # Close the figure to free up memory
```
...
if not st.session_state.rf_model_trained:
    ...
    if st.button("Perform Cost-Sensitive Optimization"):
        with st.spinner("Calculating optimal thresholds and costs..."):
            ...
            st.session_state.thresholds_costs_data = thresholds_costs # Store tuple (thresholds, costs_log, costs_rf)
    ...
    if st.button("Assess and Calibrate PDs"):
        with st.spinner("Performing PD calibration and Brier score calculation..."):
            y_prob_rf_calibrated, brier_log, brier_rf, brier_rf_calibrated = perform_pd_calibration(
                ...
                st.session_state.y_prob_rf,
                st.session_state.best_rf,
                # Assuming X_train_sm, y_train_sm are available globally from source.py or re-generated
                # For this blueprint, assuming a modified perform_pd_calibration in source.py handles SMOTE internally or takes X_train_sc, y_train
                st.session_state.X_train_sc, # Re-use un-SMOTEd train for calibration if SMOTE is internal
                st.session_state.y_train,
                st.session_state.X_test_sc
            )
